...
@Controller("/events")
class EventsController @Inject constructor(backend: Backend) {

    private val eventsProducer = BoundedContextHttpJsonEventProducer(backend)

    @Get(produces = [MediaType.APPLICATION_JSON])
    fun events(parameters: HttpParameters): Mono<ObjectNode> = eventsProducer.produceFrom(parameters.asMap())
}